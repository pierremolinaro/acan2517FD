...
//---
        return begin(inSettings, filters);
    }
...
//----------------------------------- INT, CS pins, reset MCP2517FD
        if (errorCode == 0) {
            ...
            while (wait) {
                writeRegister8(CON_REGISTER + 3,
                               0x04 | (1 << 3)); // Request configuration mode, abort all transmissions
...
//······················································································································
    ...
    bool ACAN2517FD::end(void) {
        ...
        const uint32_t startTime = millis();
        while (wait) {
            writeRegister8Assume_SPI_transaction(CON_REGISTER + 3,
                                                 0x04 |
                                                 (1 << 3)); // Request configuration mode, abort all transmissions
...
//----------------------------------------------------------------------------------------------------------------------
    ...
    void ACAN2517FD::appendInControllerTxFIFO(const CANFDMessage &inMessage) {
        const uint16_t ramAddr = uint16_t(
                0x400 + readRegister32Assume_SPI_transaction(FIFOUA_REGISTER(TRANSMIT_FIFO_INDEX)));
...
//----------------------------------------------------------------------------------------------------------------------
    ...
    bool ACAN2517FD::dispatchReceivedMessage(const tFilterMatchCallBack inFilterMatchCallBack) {
        ...
        if (hasReceived) {
            ...
            ACANFDCallBackRoutine callBackFunction = (mCallBackFunctionArray == NULL) ? NULL
...
//----------------------------------------------------------------------------------------------------------------------
    ...
    void ACAN2517FD::receiveInterrupt(void) {
        const uint16_t ramAddress = uint16_t(
                0x400 + readRegister32Assume_SPI_transaction(FIFOUA_REGISTER(RECEIVE_FIFO_INDEX)));
        CANFDMessage message;
