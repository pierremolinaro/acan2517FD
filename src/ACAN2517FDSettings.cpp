...
//----------------------------------------------------------------------------------------------------------------------
    ...
    ACAN2517FDSettings::ACAN2517FDSettings(const Oscillator inOscillator,
        ...
        if (inDataBitRateFactor == DataBitRateFactor::x1) { // Single bit rate
            const uint32_t maxTQCount = MAX_ARBITRATION_PHASE_SEGMENT_1 + MAX_ARBITRATION_PHASE_SEGMENT_2 +
                                        1; // Setting for slowest bit rate
            ...
            while ((TQCount <= (MAX_ARBITRATION_PHASE_SEGMENT_1 + MAX_ARBITRATION_PHASE_SEGMENT_2 + 1)) && (BRP > 0)) {
                ...
                if ((TQCount >= 4) && (TQCount <= maxTQCount)) {
                    const uint32_t error =
                            mSysClock - inDesiredArbitrationBitRate * TQCount * BRP; // error is always >= 0
                ...
                if ((TQCount >= 3) && (TQCount < maxTQCount)) {
                    const uint32_t error =
                            inDesiredArbitrationBitRate * (TQCount + 1) * BRP - mSysClock; // error is always >= 0
                    if (error <= smallestError) {
                        smallestError = error;
                        bestBRP = BRP;
            ...
            const uint32_t maxDataTQCount =
                    MAX_DATA_PHASE_SEGMENT_1 + MAX_DATA_PHASE_SEGMENT_2; // Setting for slowest bit rate
            ...
            while ((dataTQCount <= maxDataTQCount) && (brp > 0)) {
                ...
                if (brp < MAX_BRP) {
                    const uint32_t error =
                            desiredDataBitRate * dataTQCount * (brp + 1) - mSysClock; // error is always >= 0
                    if (error <= smallestError) {
                        smallestError = error;
                        bestBRP = brp + 1;
                        bestDataTQCount = dataTQCount;
...
//----------------------------------------------------------------------------------------------------------------------
    ...
    bool ACAN2517FDSettings::exactDataBitRate(void) const {
        if (mDataBitRateFactor == DataBitRateFactor::x1) {
            ...
            return mSysClock ==
                   (mBitRatePrescaler * mDesiredArbitrationBitRate * TQCount * uint8_t(mDataBitRateFactor));
        }
    }
...
//----------------------------------------------------------------------------------------------------------------------
    ...
    bool ACAN2517FDSettings::dataBitRateIsAMultipleOfArbitrationBitRate(void) const {
        ...
        if (!result) {
            const uint32_t dataTQCount = 1 /* Sync Seg */ + mDataPhaseSegment1 + mDataPhaseSegment2;
            const uint32_t arbitrationTQCount =
                    1 /* Sync Seg */ + mArbitrationPhaseSegment1 + mArbitrationPhaseSegment2;
